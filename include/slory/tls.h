...
namespace nodepp { class slory_tls_t {
protected:
...
public:
    ...
    slory_tls_t ( slory_config_t args ) noexcept : obj( type::bind(args) ) { obj->state = 1; }

    virtual ~slory_tls_t () noexcept { if( obj.count()>1 ) { return; } free(); }

    slory_tls_t () noexcept : obj( new slory_config_t() ){}
    ...
    auto self = type::bind( this ); onOpen.emit(); ssl_t ssl;

        process::add( coroutine::add( COROUTINE(){
            if( self->is_closed() ){ self->close(); return -1; }
        coBegin ; coWait( !limit::fileno_ready() ); do {

            auto skt = tls::client( ssl ); skt.onConnect.once([=]( ssocket_t cli ){
...
namespace nodepp { namespace slory {
    ...
    slory_tls_t tls( const slory_config_t& args ){ 
    slory_tls_t pid( args ); pid.pipe(); return pid;
    }
